...
export default class ShelterService {
  static async createShelter(
    ...
    try {
      ...
      const shelterCreated: IShelter = await shelterDao.create(shelterPayload);
  ...
  static async updateShelter(
    ...
    try {
      ...
      const { updatedAt: _, ...shelterFoundWithoutUpdatedAt } =
        shelterFound.toObject ? shelterFound.toObject() : shelterFound;
      ...
      if (
        JSON.stringify(shelterFoundWithoutUpdatedAt) ===
        JSON.stringify(shelterPayload)
      ) {
        throw new HttpError(
          "No changes detected",
          "NO_CHANGES",
      ...
      const updatedShelter = await shelterDao.update(user.id, shelterPayload);
      if (!updatedShelter) {
        throw new HttpError(
          "shelter not updated",
          "SHELTER_NOT_UPDATED",
  ...
  static async getShelterById(id: string): Promise<Partial<ShelterResponse>> {
    try {
      const shelterDao = new ShelterDAO();
