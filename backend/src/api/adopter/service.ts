...
export default class AdopterService {
  static async createAdopter(
    ...
    try {
      ...
      const adopterCreated: IAdopter = await adopterDao.create(adopterPayload);
  ...
  static async getAdopterById(id: string): Promise<Partial<AdopterResponse>> {
    try {
  ...
  static async updateAdopter(
    ...
    try {
      ...
      const adopterObj = adopterFound.toObject
        ? adopterFound.toObject()
        : adopterFound;
      ...
      if (
        JSON.stringify(adopterFoundWithoutUpdatedAt) ===
        JSON.stringify(adopterPayload)
      ) {
        throw new HttpError(
          "No changes detected",
          "NO_CHANGES",
