...
const VerticalCard = ({image, title, description, isPaused = false, onEdit, onPause, onSee}) => {
  ...
  return (
    <div className={` ${isPaused ? 'bg-(--accent-ultradark)' : 'bg-(--accent-dark)'} w-2xs mx-auto rounded-lg shadow-lg`}>
      <div className='relative'>
        <img src={image} alt='' className={`${isPaused ? 'grayscale' : ''} object-cover object-center w-full rounded-t-lg h-62 bg-gray-500`} />
        {userRole === "shelter" && (
          <button className='absolute top-2 right-2 bg-(--accent-dark) text-black rounded-full p-1 hover:bg-(--primary)' onClick={onEdit}>
        ...
        {isPaused && (
          <div className='absolute inset-0 z-10 w-fit h-fit my-auto mx-4 py-2 px-2 bg-black/85 text-(--accent) rounded-xl'>
            <p className='font-normal text-center'>
              PUBLICACIÓN PAUSADA
            </p>
            <p className='text-center'>No se mostrará a posibles adoptantes</p>
          </div>
        )}
      </div>
